...
const PlayerModal = (function(){
  ...
  function loadPlayerDetails(type, id, season = '1', episode = '1') {
    try {
      ...
      if (type === 'tv' && id) {
        frame.src = `https://player.videasy.net/tv/${encodeURIComponent(id)}/${encodeURIComponent(season)}/${encodeURIComponent(episode)}?Play=true&color=e50914&nextEpisode=true&episodeSelector=true`;
      } else if (type === 'movie' && id) {
        frame.src = `https://player.videasy.net/movie/${encodeURIComponent(id)}?&Play=true&color=e50914`;
      }
