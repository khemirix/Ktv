  ...
  function loadPlayerDetails(type, id, season = '1', episode = '1') {
    try {
      ...
      if (type === 'tv' && id) {
        ...
        frame.src = `https://player.videasy.net/tv/${encodeURIComponent(id)}/${encodeURIComponent(season)}/${encodeURIComponent(episode)}?overlay=true&autoplay=true&color=e50914&nextEpisode=true&episodeSelector=true`;
      } else if (type === 'movie' && id) {
        // Ensure the iframe can enter fullscreen manually via user interaction
        frame.setAttribute('allow', (frame.getAttribute('allow') || '') + ' fullscreen; autoplay');
        frame.setAttribute('allowfullscreen', '');
        frame.src = `https://player.videasy.net/movie/${encodeURIComponent(id)}?overlay=true&autoplay=true&color=e50914`;
      }
